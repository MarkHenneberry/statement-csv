...
export const coordinateSamples: CoordSample[] = [
  ...
  // E. Multi-line FX credit-card row: merchant, reference, FX detail, final CAD amount
  {
    ...
    rows: [
      ...
      [["APR 12", X.date], ["APR 13", X.postDate], ["GLOBAL WEBSHOP USA", X.desc]],
  ...
  // N. Credit-card FX detail line before the final CAD amount.
  {
    ...
    rows: [
      ...
      [["Trans Date", X.date], ["Post Date", X.postDate], ["Description", X.desc], ["Amount", X.amount]],
      [["APR 12", X.date], ["APR 13", X.postDate], ["GLOBAL WEBSHOP", X.desc]],
  ...
  // U. CC continued across pages where page 2 adds an optional category column.
  {
    name: "U-cc-optional-column-stitch",
    description: "Page 2 inserts an optional Category column; core columns still stitch (relaxed)",
    rows: [
      [["RBC Visa", X.date]],
      [["Previous Account Balance", X.date], ["$100.00", X.amount]],
      [["Transaction", X.date], ["Date", X.date + 65], ["Posting", X.postDate], ["Date", X.postDate + 65], ["Description", X.desc], ["Amount", X.amount]],
    ...
    morePages: [
      [
        [["Transaction", X.date], ["Date", X.date + 65], ["Posting", X.postDate], ["Date", X.postDate + 65], ["Description", X.desc], ["Category", X.category], ["Amount", X.amount]],
        [["APR 10", X.date], ["APR 11", X.postDate], ["STORE C", X.desc], ["Shopping", X.category], ["20.00", X.amount]],
        [["APR 12", X.date], ["APR 13", X.postDate], ["PAYMENT THANK YOU", X.desc], ["Payment", X.category], ["100.00-", X.amount]],
        [["New Balance", X.date], ["$100.00", X.amount]],
      ],
    ],
    expect: {
      ...
      stitched: true,
      regionsStitched: 2,
      note: "optional category column on page 2 only; relaxed stitch keeps the table together",
    },
  },

  // V. CC over-capture trap: statement metadata near the table must be excluded.
  {
    name: "V-cc-overcapture-trap",
    description: "A summary balance line and a dateless metadata amount must not become rows",
    rows: [
      [["RBC Visa", X.date]],
      [["Previous Account Balance", X.date], ["$100.00", X.amount]],
      [["Transaction", X.date], ["Date", X.date + 65], ["Posting", X.postDate], ["Date", X.postDate + 65], ["Description", X.desc], ["Amount", X.amount]],
      [["APR 02", X.date], ["APR 03", X.postDate], ["STORE A", X.desc], ["50.00", X.amount]],
      [["APR 04", X.date], ["APR 05", X.postDate], ["STORE B", X.desc], ["30.00", X.amount]],
      // Trap 1: a known summary label with an amount (ignored by label).
      [["Total account balance", X.desc], ["180.00", X.amount]],
      // Trap 2: dateless metadata with an amount in the amount column (guard-rejected).
      [["Points earned this period", X.desc], ["500.00", X.amount]],
      [["New Balance", X.date], ["$180.00", X.amount]],
    ],
    expect: {
      rows: 2,
      opening: 100,
      ...
      statementKind: "credit-card",
      stitched: false,
      noDescriptionIncludes: ["Total account balance", "Points", "500.00"],
      note: "summary label ignored; dateless amount rejected as non-transaction",
    },
  },

  // W. CC interest-rate chart after the transactions must not be captured.
  {
    name: "W-cc-interest-rate-chart",
    description: "An interest-rate chart (percentages, no dates) after the rows is not transactions",
    rows: [
      [["RBC Visa", X.date]],
      [["Previous Account Balance", X.date], ["$100.00", X.amount]],
      [["Transaction", X.date], ["Date", X.date + 65], ["Posting", X.postDate], ["Date", X.postDate + 65], ["Description", X.desc], ["Amount", X.amount]],
      [["APR 02", X.date], ["APR 03", X.postDate], ["STORE A", X.desc], ["50.00", X.amount]],
      [["APR 04", X.date], ["APR 05", X.postDate], ["STORE B", X.desc], ["30.00", X.amount]],
      [["INTEREST RATE CHART", X.desc]],
      [["Purchases", X.desc], ["19.99 %", X.amount]],
      [["Cash Advances", X.desc], ["22.99 %", X.amount]],
      [["New Balance", X.date], ["$180.00", X.amount]],
    ],
    expect: {
      rows: 2,
      ...
      statementKind: "credit-card",
      stitched: false,
      noDescriptionIncludes: ["INTEREST", "19.99", "22.99"],
      note: "interest-rate percentages are not money and carry no date; never transactions",
    },
  },

  // X. CC subtotal rows interspersed in a single region must not become transactions.
  {
    name: "X-cc-subtotal-rows",
    description: "SUBTOTAL rows between transactions are section hints, not transactions",
    rows: [
      [["RBC Visa", X.date]],
      [["Previous Balance", X.date], ["$0.00", X.amount]],
      [["Trans Date", X.date], ["Post Date", X.postDate], ["Description", X.desc], ["Amount", X.amount]],
      [["APR 05", X.date], ["APR 06", X.postDate], ["STORE A", X.desc], ["50.00", X.amount]],
      [["SUBTOTAL OF MONTHLY ACTIVITY", X.desc], ["50.00", X.amount]],
      [["APR 10", X.date], ["APR 11", X.postDate], ["STORE B", X.desc], ["30.00", X.amount]],
      [["SUBTOTAL OF MONTHLY ACTIVITY", X.desc], ["80.00", X.amount]],
      [["New Balance", X.date], ["$80.00", X.amount]],
    ],
    expect: {
      rows: 2,
      ...
      stitched: false,
      noDescriptionIncludes: ["SUBTOTAL"],
      note: "subtotal rows are ignored (validation hints), not transactions",
    },
  },

  // Y. CC with combined transaction+posting date in ONE date cell (no postDate col).
  {
    name: "Y-cc-combined-date-cell",
    description: "Both dates collapse into one date column; the transaction date must be recovered",
    rows: [
      [["RBC Visa", X.date]],
      [["Previous Balance", X.date], ["$100.00", X.amount]],
      [["Date", X.date], ["Description", X.desc], ["Amount", X.amount]],
      [["APR 22", X.date], ["APR 24", X.date + 30], ["STORE A", X.desc], ["50.00", X.amount]],
      [["APR 25", X.date], ["APR 27", X.date + 30], ["STORE B", X.desc], ["30.00", X.amount]],
      [["New Balance", X.date], ["$180.00", X.amount]],
    ],
    expect: {
      rows: 2,
